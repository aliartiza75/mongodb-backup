...
## NOTE
# To store the status of backup process telegraf is used. Telegraf reads mongo-backup.stat file 
# that will contains the status of the backup process and injest it in a measurement of the influx
# db's database. 
# Two methods has been written that will be called after backup commands. To check whether command executed successfully or not.
